...
module Depends (findDependHS) where
...
pp :: (GhcMonad m, Outputable a) => a -> m ()
pp a = do
  dflags <- GHC.getSessionDynFlags
  liftIO . putStrLn . showSDoc dflags . ppr $ a

-- fp should be rooted at the cabal file.
-- "/usr/lib/ghc" will do for now, but it should get the value from runghc somehow
findDependHS :: [FilePath] -> IO [Maybe FilePath]
findDependHS fps = GHC.runGhc (Just "/usr/lib/ghc") (doFindDependHS fps)

doFindDependHS :: GhcMonad m => [FilePath] -> m [Maybe FilePath]
doFindDependHS srcs = do
    -- Initialisation
    dflags <- GHC.getSessionDynFlags
    ...
    return $ map (ml_hs_file . ms_location) mod_summaries
...
pprCycle summaries = pp_group (CyclicSCC summaries)
  where
    ...
    pp_group (CyclicSCC mss)
        = if debugIsOn && not (( not (null boot_only) )) then (assertPanic "Depend.hs" 356) else
