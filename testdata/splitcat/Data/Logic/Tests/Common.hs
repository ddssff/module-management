...
import Data.Logic.Classes.FirstOrder (convertFOF, FirstOrderFormula)
import Data.Logic.Classes.Literal (Literal)
import Data.Logic.Classes.Pretty (Pretty(pretty))
import Data.Logic.Classes.Propositional (PropositionalFormula)
import qualified Data.Logic.Classes.Skolem as C (Skolem(..))
import Data.Logic.Classes.Term (Function, Term(vt, fApp, foldTerm))
import Data.Logic.Classes.Variable (Variable(..))
import Data.Logic.Harrison.Normal (trivial)
import Data.Logic.Harrison.Skolem (nnf, pnf, runSkolem, simplify, Skolem, skolemize)
import qualified Data.Logic.Instances.Chiou as Ch (CTerm, Sentence(Equal, Predicate))
import Data.Logic.Instances.PropLogic (plSat)
import qualified Data.Logic.Instances.SatSolver as SS (toCNF)
import Data.Logic.KnowledgeBase (getKB, loadKB, Proof, runProver', theoremKB, WithId)
...
import qualified Data.Set as S (empty, fromList, insert, map, member, Set, toList)
import Data.String (IsString(fromString))
import Test.HUnit (assertEqual, Test(..))
import Text.PrettyPrint ((<>), Doc, Mode(OneLineMode), renderStyle, style, Style(mode), text)
