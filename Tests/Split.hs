...
split4 =
    ...
    do _ <- system "rsync -aHxs --delete testdata/split4/ tmp"
       withCurrentDirectory "tmp" $
         runMonadClean $ modifyTestMode (const True) >> putName (S.ModuleName "Split4") >> splitModuleDecls "Split4.hs"
...
split4b =
    ...
    do _ <- system "rsync -aHxs --delete testdata/split4/ tmp"
       withCurrentDirectory "tmp" $
         runMonadClean $ modifyTestMode (const True) >> putName (S.ModuleName "Split4") >> splitModule f "Split4.hs"
