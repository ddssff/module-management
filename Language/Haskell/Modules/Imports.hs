       ...
       case source of
         ParseOk (m@(A.Module _ h _ imports _decls), comments) ->
             do let name = case h of
                ...
                dumpImports path >> checkImports path name m comments (hiddenImports ++ standaloneDerivingImports m)
...
-- source file.
checkImports :: MonadClean m => FilePath -> S.ModuleName -> A.Module SrcSpanInfo -> [Comment] -> [A.ImportDecl SrcSpanInfo] -> m ModuleResult
checkImports path name@(S.ModuleName name') m comments extraImports =
    do let importsPath = name' <.> ".imports"
       ...
       case result of
         ParseOk newImports -> updateSource path m comments newImports name extraImports
...
updateSource :: MonadClean m => FilePath -> A.Module SrcSpanInfo -> [Comment] -> A.Module SrcSpanInfo -> S.ModuleName -> [A.ImportDecl SrcSpanInfo] -> m ModuleResult
updateSource path (m@(A.Module _ _ _ oldImports _)) comments (A.Module _ _ _ newImports _) name extraImports =
    do remove <- removeEmptyImports <$> getParams
       ...
       maybe (qPutStrLn ("cleanImports: no changes to " ++ path) >> return (Unchanged name))
             ...
             (replaceImports (fixNewImports remove oldImports (newImports ++ extraImports)) m comments text)
updateSource _ _ _ _ _ _ = error "updateSource"

-- | Compare the old and new import sets and if they differ clip out
-- the imports from the sourceText and insert the new ones.
replaceImports :: [A.ImportDecl SrcSpanInfo] -> A.Module SrcSpanInfo -> [Comment] -> String -> Maybe String
replaceImports newImports m comments sourceText =
    let oldPretty = foldImports (\ _ pref s suff r -> r <> pref <> s <> suff) m comments sourceText ""
        -- Surround newPretty with the same prefix and suffix as oldPretty
        newPretty = fromMaybe "" (foldImports (\ _ pref _ _ r -> maybe (Just pref) Just r) m comments sourceText Nothing) <>
                    intercalate "\n" (map (prettyPrintWithMode (defaultMode {layout = PPInLine})) newImports) <>
                    foldImports (\ _ _ _ suff _ -> suff) m comments sourceText "" in
    if oldPretty == newPretty
    then Nothing
    else Just (foldHeader (\ s r -> r <> s) (\ _ pref s suff r -> r <> pref <> s <> suff) (\ _ pref s suff r -> r <> pref <> s <> suff) (\ _ pref s suff r -> r <> pref <> s <> suff) m comments sourceText "" ++
               foldExports (\ s r -> r <> s) (\ _ pref s suff r -> r <> pref <> s <> suff) (\ s r -> r <> s) m comments sourceText "" ++
               newPretty <>
               foldDecls  (\ _ pref s suff r -> r <> pref <> s <> suff) (\ r s -> r <> s) m comments sourceText "")
