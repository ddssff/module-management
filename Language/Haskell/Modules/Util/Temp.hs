...
withTempDirectory :: (MonadIO m, MonadBaseControl IO m) =>
                     Bool -- ^ Remove temp directory on completion?
                  -> FilePath -- ^ Temp directory to create the directory in
                  -> String   -- ^ Directory name template. See 'openTempFile'.
                  -> (FilePath -> m a) -- ^ Callback that can use the directory
                  -> m a
withTempDirectory cleanup targetDir template =
    IOT.bracket
       (liftIO $ Temp.createTempDirectory targetDir template)
       (if cleanup then const (pure ()) else liftIO . ignoringIOErrors . removeDirectoryRecursive)
