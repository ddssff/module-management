...
import qualified CLI.Cabal as Cabal (getModules, getSrcDirs, readPackageDescription, showGenericPackageDescription, update, writeGenericPackageDescription)
import CLI.HaskelineTransAdapter ()
import Control.Exception (fromException)
import Control.Lens ((%=), (.=), use)
import Control.Monad as List (mapM_)
import Control.Monad.Reader (liftM2, MonadIO(liftIO), MonadReader(ask, local), MonadTrans(lift), ReaderT(runReaderT), unless, when)
...
import qualified Data.Set as Set (fromList, map, member, toList)
...
import System.Console.Haskeline (catch, completeFilename, CompletionFunc, defaultSettings, Exception, getInputLine, InputT, mapInputT, noCompletion, runInputT, setComplete, simpleCompletion, SomeException(SomeException), throwIO)
