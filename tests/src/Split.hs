...
import Language.Haskell.Modules (noisily, putHsSourceDirs, putModule, runImportsT, splitModule, splitModuleBy, withCurrentDirectory, findHsModules, extraImport)
import Language.Haskell.Modules.Split (T(..), splitModuleDecls)
...
tests :: Test
tests = TestList [split1 {-split2a, split2b, split4, split4b, split4c, split5-} {-, split6-}]
...
split1 =
    ...
    where
      f :: Maybe S.Name -> T -> S.ModuleName
      f (Just name) (A decl)
          | elem name (map S.Ident ["pprint1", "pprintW", "pprintL", "pprintStyle", "friendlyNames"]) =
              trace ("Move to Split2: " ++ show name) (S.ModuleName "Split2")              -- Move the pretty printing functions to Split2
      -- f Nothing (A decl) = S.ModuleName "Split2" -- Move all instances to Split2
      f (Just name) (A decl) = trace ("Leave in Split1: " ++ show name) (S.ModuleName "Split1")
      f Nothing (A decl) = trace ("Leave in Split1: " ++ show decl) (S.ModuleName "Split1")
